...
const catPost = async (
  ...
  // catPost should use getUser function from userModel
  // catPost should use res.json to send response
  // catPost should use res.sendStatus to send status code
  // catPost should use next to pass error to error handler
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    const messages: string = errors
      .array()
  ...
  try {
    const cat = req.body;
    //Get user id and role from database using id
...
const catPut = async (
  ...
  try {
    ...
    const result = await updateCat(cat, id, user.user_id, user.role);
    res.json(result);
  } catch (error) {
    next(error);
  }
